...
https://www.askpython.com/python-modules/pandas/pandas-read-sql
...
class Infostorage():
    ...
    def __init__(self):
        ...
        self.df: pandas.core.frame.DataFrame = None

    def test_setup(self):
        self.name_of_table_imgui = "title of the tab from class Infostorage"
        self.list_col_names = ["ID", "File", "Size", "Last_modified", "Lets_make_a_pie"]
        ...
        print("from Infostorage.test_setup")
        print(self.list_of_rows)

    def make_dataframe_from_list_of_list(self):
        # https://www.geeksforgeeks.org/creating-pandas-dataframe-using-list-of-lists/

        self.df = pd.DataFrame(self.list_of_rows, columns = self.list_col_names)
        print(f"\n  type(self.df) = {type(self.df)}   \n ")
        print(self.df)
    ...
    def pie_col_index(self):
        ...
        return self.list_col_names.index("Lets_make_a_pie")
...
myInfostorage.make_dataframe_from_list_of_list()
...
def look_db(returned):
    ...
    last_col_name = myInfostorage.list_col_names[-1]

    print(f"  last_col_name  =  {last_col_name}")


    

    # q1 = """SELECT ID FROM df """
    #input_query = """SELECT ID FROM df """
    input_query = f"""SELECT {last_col_name} FROM df """

    print("-----------------------------------------------")

    print(ps.sqldf(input_query, locals()))
...
class InputBar(Section):
    ...
    def on_resize(self, width: int, height: int):
        # stick to the top
        self.width = width
        self.bottom = height - self.view.inputbar_section.height
...
class GameView(arcade.View):
    def __init__(self):
        ...
        self.cell_modal_section = CellModalSection(166,
                                          (266) - 100,
                                          350, 180)
        ...
        self.data_section = Datasec(0, 0, SCREEN_WIDTH, 290)
        ...
        self.icon_section = IconSection (200,140 ,SCREEN_WIDTH, 140)        
        ...
        #self.inputbar_section = InputBar(0, self.window.height - INPUTBAR_HEIGHT + 300,
        #self.inputbar_section = InputBar(left=0, bottom=300, width=SCREEN_WIDTH, height=50,                          
        #                        accept_keyboard_events=False)

        self.inputbar_section = InputBar(90, 700, SCREEN_WIDTH, 290)

        #self.section_manager.add_section(self.inputbar_section)
        ...
        "inputbar_section":self.inputbar_section,
...
class SpecialWindow(arcade.Window):
    ...
    def __init__(self):
        ...
        super().__init__(SCREEN_WIDTH, SCREEN_HEIGHT, "init SpecialWindow Basic Example", resizable=True)
